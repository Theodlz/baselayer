...
from baselayer.tools.api import api

if __name__ == "__main__":
    parser.description = 'Load data into the database'
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=textwrap.dedent(
            '''Fully specified URI of the running instance.
    ...
    parser.add_argument(
        '--token',
        help=textwrap.dedent(
            '''Token required for accessing the API.

                             By default, the application produces a token that is
    ...
    for i in range(RETRIES):
        try:
            ...
            if admin_token != previous_admin_token:
                print('Loaded token from .tokens.yaml')
            ...
            if admin_token:
                ...
                print('auto-generated .tokens.yaml')
                time.sleep(timeout)
                continue
            ...
            if i == RETRIES - 1:
                print('FAIL')
                print()
                print('Error: Could not connect to instance; please ensure ')
    ...
    if data['status'] != 'success':
        print(
            'Error: Could not authenticate; please specify a valid token.'
        )
        sys.exit(-1)
        ...
        if isinstance(obj, dict):
            if 'file' in obj:
                ...
                if filename.endswith('csv'):
                    ...
                    if Regions is None:
                        raise ImportError(
                            'regions is required to load region files; '
                            'please install it using `pip install regions`'
                        )
                    return Regions.read(filename).serialize(format='ds9')
                elif filename.endswith('h5') or filename.endswith('hdf5'):
                    if Table is None:
                        raise ImportError(
                            'astropy is required to load HDF5 files; '
                            'please install it using `pip install astropy`'
                        )
                    try:
                        payload = (
                            Table.read(filename)
